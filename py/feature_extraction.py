...
class FeatureExtract:
    ...
    Args:
        ...
        snr (float): The snr for the data.
    ...
    def generate_feature(
        ...
        Args:
            snr (float): The snr for the data.
            feature_type (str): type of features (log-mel / power-spec)
            nfft (int): length of STFT frame
            hop_length (int): number of samples to hop between adjacent frames in output
            wincount (int): total number of frames
            nfilters (int): number of log-mel filters
        ...
        if feature_type == "log-mel":
            ...
            raise ValueError(f"unknown feature set {feature_type}")
        
        n_windows = S.shape[-1] // wincount
        # cut out remainder of frames
        S = S[:, 0:n_windows*wincount] 
        # transpose so that frequency is on the fast axis
        feature_data = S.transpose()
